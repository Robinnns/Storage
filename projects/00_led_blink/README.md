...
**核心思想：** 设计（`src/` `sim/`）与 板卡（`fpga/`）与 工具（`eda/`）三层解耦。
...
脚本自动完成：建工程（器件 `xc7a35tfgg484-2`）→ 引用 `src/`、`sim/`、`fpga/a7_lite/` 外部文件 → 设顶层 `top` → 保存到 `workspace/`。
...
2. Add Sources → Add Directories → 选 `src/`
3. Add Constraints → Add Files → 选 `fpga/a7_lite/a7_lite.xdc`
4. Add Simulation Sources → Add Files → 选 `sim/tb_top.v`
...
```bash
# 若已装 Icarus Verilog
iverilog -g2012 -o tb.vvp src/top.v sim/tb_top.v
vvp tb.vvp
```
